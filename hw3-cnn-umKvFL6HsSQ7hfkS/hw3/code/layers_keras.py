...
class Conv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.kernel_initializer(shape=[self.kernel_size[0], self.kernel_size[1], 
        input_shape[-1], self.filters], dtype=tf.float32)
        
        self.bias = self.bias_initializer(shape=[self.filters], dtype=tf.float32)
    ...
    def call(self, inputs, training=False):
        ...
        out = tf.nn.convolution(inputs, self.kernel, self.strides, self.padding)
        ...
        self.add_loss(self.activity_regularizer(out))
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.beta = self.beta_initializer(shape=1, dtype=tf.float32) * self.center
        self.gamma = self.gamma_initializer(shape=1, dtype=tf.float32) * self.scale
        self.moving_mean = self.moving_mean_initializer(shape=[input_shape[self.axis]], dtype=tf.float32)
        self.moving_variance = self.moving_variance_initializer(shape=[input_shape[self.axis]], dtype=tf.float32)
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            self.moving_mean.assign(self.momentum * self.moving_mean + (1 - self.momentum) * bmu)
            self.moving_variance.assign(self.momentum * self.moving_variance + (1 - self.momentum) * bvar)
        else: 
            bmu = self.moving_mean
            bvar = self.moving_variance

        x = self.gamma * (inputs - bmu) / tf.sqrt(bvar + self.epsilon) + self.beta
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            if self.noise_shape is not None:
                mask = tf.random.uniform(shape=self.noise_shape, minval=0, maxval=1) <= self.rate
            else: 
                mask = tf.random.uniform(tf.shape(inputs), minval=0, maxval=1, seed=self.seed) >= self.rate
